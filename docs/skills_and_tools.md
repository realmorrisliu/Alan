...
- **Static shell paths only** — the workspace path guard rejects shell variable, command, and brace expansion because dynamic paths cannot be validated safely before `sh -c` runs
