 ...
 *
 * 配置文件路径：
 * - 生产模式: ~/.alan/config.toml
 * - 开发模式: 项目根目录下的 config.toml（如果存在）
 *
...
function getConfigPath(): string | null {
  ...
  // 2. 生产模式：~/.alan/config.toml
  const prodConfig = join(homedir(), ".alan", "config.toml");
  ...
  const devConfig = join(rootDir, "config.toml");
...
export class DaemonManager {
  ...
  async start(): Promise<DaemonStatus> {
    ...
    if (!configPath) {
      ...
      if (mode === "production") {
        warnMsg += "请创建配置文件:\n";
        warnMsg += `   vim ~/.alan/config.toml\n\n`;
        warnMsg += "或使用模板:\n";
        warnMsg += `   mkdir -p ~/.alan\n`;
        warnMsg += `   cp ~/.alan/config.toml.example ~/.alan/config.toml\n`;
      } else {
        warnMsg += "请创建配置文件（项目根目录或 ~/.alan/config.toml）\n";
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.waitForReady(this.config.startupTimeout)
        ...
        .catch((error) => {
          ...
          if (
            logs.includes("LLM") ||
            logs.includes("Gemini") ||
            logs.includes("OpenAI")
          ) {
            enhancedError +=
              "\n\n提示: 看起来 agentd 需要 LLM 配置。请设置以下环境变量之一:\n";
            enhancedError +=
              "  - Gemini: LLM_PROVIDER=gemini GEMINI_PROJECT_ID=your-project\n";
            enhancedError +=
              "  - OpenAI: LLM_PROVIDER=openai_compatible OPENAI_COMPAT_API_KEY=your-key\n";
            enhancedError +=
              "  - Anthropic: LLM_PROVIDER=anthropic_compatible ANTHROPIC_COMPAT_API_KEY=your-key\n";
          }
