...
- [x] 5.5 Manually verify shell scrollback, alternate-screen scrolling, selection/copy, paste, right-click, IME composition, terminal mouse apps, search, child exit, and renderer/fallback state, using focused fake-surface/runtime tests where live UI automation cannot safely exercise pasteboard, IME preedit, child-exit, or renderer-failure states.
...
- [x] 6.3 Before archive, remove or split deferred delta requirements for live secure-input callbacks, URL hover, and dedicated bracketed paste, then sync accepted delta requirements into `openspec/specs/`.
