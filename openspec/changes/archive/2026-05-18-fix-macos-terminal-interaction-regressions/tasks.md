...
- [x] 1.1 在当前 macOS app 中复现 Vim 快捷键问题，记录被截获的具体按键、focused pane、command input 可见状态和相关日志。
  - 2026-05-18: Post-fix user verification confirmed the Vim/nvim keyboard path is OK; this is no longer a live blocker.
...
- [x] 5.2 构建并运行 macOS app，手工验证 Vim/nvim 的插入、退出、移动、保存/退出等快捷键路径。
  - 2026-05-18: User confirmed the Vim/nvim path is OK in the running app.
...
- [x] 5.6 实现合入后，将 delta specs 同步到 `openspec/specs/` 并准备 archive。
